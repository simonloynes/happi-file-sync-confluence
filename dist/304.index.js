...
const colorConvert = __webpack_require__(7414);
...
module.exports =
{
  parallel      : __webpack_require__(5139),
  serial        : __webpack_require__(479),
  serialOrdered : __webpack_require__(5250)
};
...
var defer = __webpack_require__(6695);
...
var async = __webpack_require__(5455)
  , abort = __webpack_require__(6233)
  ;
...
var abort = __webpack_require__(6233)
  , async = __webpack_require__(5455)
  ;
...
var iterate    = __webpack_require__(8577)
  , initState  = __webpack_require__(4714)
  , terminator = __webpack_require__(6498)
  ;
...
var serialOrdered = __webpack_require__(5250);
...
var bind = __webpack_require__(8833);

var $apply = __webpack_require__(4894);
var $call = __webpack_require__(3184);
var $reflectApply = __webpack_require__(6499);
...
var bind = __webpack_require__(8833);
var $TypeError = __webpack_require__(7901);

var $call = __webpack_require__(3184);
var $actualApply = __webpack_require__(1892);
...
const escapeStringRegexp = __webpack_require__(8799);
const ansiStyles = __webpack_require__(4527);
const stdoutColor = (__webpack_require__(9823).stdout);

const template = __webpack_require__(8142);
...
/* MIT license */
var cssKeywords = __webpack_require__(7594);
...
var conversions = __webpack_require__(1923);
var route = __webpack_require__(1523);
...
var DelayedStream = __webpack_require__(6317);
...
const marked_1 = __webpack_require__(3055);
const path = __importStar(__webpack_require__(6928));
const escapeXmlCharacters = (s) => {
    return s
        .replace(/&/g, "&amp;")
        .replace(/</g, "&lt;")
        .replace(/>/g, "&gt;")
...
class ConfluenceRenderer extends marked_1.Renderer {
    constructor(options, config, page) {
        ...
        this.DEFAULT_LANGUAGE_FOR_CODE_BLOCK = "none";
        this.config = config;
        this.options = options;
        this.page = page;
    }
    static hasDetailsBlock(html) {
        return !!html.match(/<details>([\s\S]*)<\/details>/);
    }
    renderDetailsBlock(html) {
        var _a, _b;
        const summary = (_b = (_a = html.match(/<summary>([\s\S]*)<\/summary>/)) === null || _a === void 0 ? void 0 : _a[1]) !== null && _b !== void 0 ? _b : "Click here to expand ...";
        const contentWithoutSummaryTags = html
            .replace(/<summary>([\s\S]*)<\/summary>/, "")
            .replace(/<\/?details>/g, "");
        const content = (0, marked_1.marked)(contentWithoutSummaryTags, {
    ...
    link(href, title, text) {
        ...
        return super.link(escapeXmlCharacters(href !== null && href !== void 0 ? href : ""), title, text);
    }
    resolveLinks(href) {
        ...
        if (match) {
            href = `${this.config.baseUrl.replace("rest/api", "").replace(/\/$/, "")}/pages/viewpage.action?pageId=${match.pageId}`;
        }
        return href;
    }
    html(html) {
    ...
    code(code, lang = this.DEFAULT_LANGUAGE_FOR_CODE_BLOCK) {
        lang =
...
//# sourceMappingURL=ConfluenceRenderer.js.map
...
class ObjectConfigLoader {
    ...
    static async fetchCredentialsFromObject(objectConfig) {
    ...
    static createAuthorizationToken(authOptions) {
    ...
    static normalizeFilePaths(objectConfig) {
        for (const i in objectConfig.pages) {
            objectConfig.pages[i].file = path_1.default.isAbsolute(objectConfig.pages[i].file)
                ? objectConfig.pages[i].file
                : path_1.default.resolve(path_1.default.dirname(objectConfig.fileRoot) + "/" + objectConfig.pages[i].file);
        }
        return objectConfig;
    }
    static createConfig(objectConfig, authorizationToken) {
...
exports.ObjectConfigLoader = ObjectConfigLoader;
//# sourceMappingURL=ObjectConfigLoader.js.map
...
const ConfluenceRenderer_1 = __importDefault(__webpack_require__(6391));
const path = __importStar(__webpack_require__(6928));
const signale_1 = __importDefault(__webpack_require__(8784));
const marked_1 = __webpack_require__(3055);
function mkdir(cachePath) {
...
function removeDynamicIds(s) {
    return s.replace(/ (ac:macro-)?id="[^"]+"/g, "");
}
function isRemoteUpdateRequired(newContent, confluencePage) {
...
function extractAttachmentsFromPage(pageData, newContent) {
    return (newContent.match(/<ri:attachment ri:filename="(.+?)" *\/>/g) || [])
        .map((attachment) => attachment.replace(/.*"(.+)".*/, "$1"))
        .filter((attachment) => !attachment.startsWith("http"))
        .filter((attachment) => {
        if (!fs.existsSync(path.resolve(path.dirname(pageData.file), attachment))) {
            signale_1.default.error(`Attachment "${attachment}" not found.`);
            return false;
        }
        return true;
    })
...
function extractTitle(fileData) {
...
function convertToWikiFormat(pageData, config) {
    ...
    const renderer = config.customRenderer
        ...
        : new ConfluenceRenderer_1.default({}, config, pageData);
    return (0, marked_1.marked)(fileData, {
        renderer,
        xhtml: true,
    });
}
function mapLocalToRemoteAttachments(attachment, remoteAttachments) {
    const remoteAttachment = remoteAttachments.find((remoteAttachment) => remoteAttachment.title === attachment.remoteFileName &&
...
async function deleteOutOfDateAttachments(attachments, remoteAttachments, confluenceAPI) {
    const upToDateAttachmentIds = attachments.map((attachment) => attachment.remoteAttachmentId);
    const outOfDateAttachments = remoteAttachments.filter((remoteAttachment) => !upToDateAttachmentIds.includes(remoteAttachment.id));
    for (const outOfDateAttachment of outOfDateAttachments) {
        await confluenceAPI.deleteAttachment(outOfDateAttachment);
    }
}
async function updateAttachments(mdWikiData, pageData, cachePath, confluenceAPI, force) {
    const remoteAttachments = (await confluenceAPI.getAttachments(pageData.pageId)).results;
    let attachments = extractAttachmentsFromPage(pageData, mdWikiData).map((attachment) => mapLocalToRemoteAttachments(attachment, remoteAttachments));
    if (!attachments) {
        return mdWikiData;
    }
    ...
    for (const attachment of attachments.filter((attachment) => force || !attachment.remoteAttachmentId)) {
        ...
        signale_1.default.await(`Uploading attachment "${attachment.remoteFileName}" for "${pageData.title}" ...`);
        try {
    ...
    mdWikiData = mdWikiData.replace(/<ri:attachment ri:filename=".+?"/g, (s) => s.replace(/(\.\.|\/)/g, "_"));
    return mdWikiData;
}
function increaseVersionNumber(versionNumber) {
    return (parseInt(versionNumber, 10) + 1).toString();
}
async function sendChangedPage(confluencePage, pageData, mdWikiData, confluenceAPI) {
    var _a;
    confluencePage.title = (_a = pageData.title) !== null && _a !== void 0 ? _a : confluencePage.title;
    confluencePage.body = {
        storage: {
            value: mdWikiData,
    ...
    signale_1.default.await(`Update page "${pageData.title}" ...`);
    await confluenceAPI.updateConfluencePage(pageData.pageId, confluencePage);
}
function addPrefix(config, mdWikiData) {
...
async function updatePage(confluenceAPI, pageData, config, force) {
    const pagePathFromConfig = pageData.file.replace(path.dirname(config.configPath) + "/", "");
    signale_1.default.start(`Starting to render "${pagePathFromConfig}"`);
    ...
    if (!force && !needsContentUpdate) {
        signale_1.default.success(`Local cache for "${pagePathFromConfig}" is up to date, no update necessary`);
        return;
    }
    mdWikiData = await updateAttachments(mdWikiData, pageData, cachePath, confluenceAPI, force);
    signale_1.default.await(`Fetch current page for "${pageData.title}" ...`);
    ...
    if (!force && !isRemoteUpdateRequired(mdWikiData, confluencePage)) {
        signale_1.default.success(`No change in remote version for "${pagePathFromConfig}" detected, no update necessary`);
        return;
    }
    ...
    signale_1.default.success(`"${confluencePage.title}" saved in confluence (${confluenceUrl}/pages/viewpage.action?pageId=${pageData.pageId}).`);
}
...
const axios_1 = __importDefault(__webpack_require__(7455));
const fs = __importStar(__webpack_require__(9896));
const signale_1 = __importDefault(__webpack_require__(8784));
const form_data_1 = __importDefault(__webpack_require__(6842));
class ConfluenceAPI {
    constructor(baseUrl, authorizationToken, insecure) {
        this.baseUrl = baseUrl;
        this.authHeader = {
            Authorization: authorizationToken,
        };
        this.agent = new ((__webpack_require__(5692).Agent))({
            rejectUnauthorized: !insecure,
        });
    }
    async updateConfluencePage(pageId, newPage) {
        var _a, _b, _c;
        try {
            await axios_1.default.put(`${this.baseUrl}/content/${pageId}`, newPage, this.config());
        }
        catch (e) {
            signale_1.default.error((_c = (_b = (_a = e === null || e === void 0 ? void 0 : e.response) === null || _a === void 0 ? void 0 : _a.data) === null || _b === void 0 ? void 0 : _b.message) !== null && _c !== void 0 ? _c : e);
            signale_1.default.await(`First attempt failed, retrying ...`);
            await axios_1.default.put(`${this.baseUrl}/content/${pageId}`, newPage, this.config());
        }
    }
    async getAttachments(pageId) {
        return (await axios_1.default.get(`${this.baseUrl}/content/${pageId}/child/attachment`, this.config())).data;
    }
    async deleteAttachment(attachment) {
        try {
            signale_1.default.await(`Deleting attachment "${attachment.title}" ...`);
            await axios_1.default.delete(`${this.baseUrl}/content/${attachment.id}`, this.config());
        }
        catch (e) {
            signale_1.default.error(`Deleting attachment "${attachment.title}" failed ...`);
        }
    }
    async uploadAttachment(filename, pageId) {
        try {
            ...
            signale_1.default.error(`Uploading attachment "${filename}" failed ...`);
        }
    }
    async currentPage(pageId) {
        return axios_1.default.get(`${this.baseUrl}/content/${pageId}?expand=body.storage,version`, this.config());
    }
    config() {
        return {
            headers: {
...
const ObjectConfigLoader_1 = __webpack_require__(7520);
const ConfluenceAPI_1 = __webpack_require__(137);
const UpdatePage_1 = __webpack_require__(8464);
const DEFAULTS = {
    insecure: false,
    force: false,
    fileRoot: process.cwd(),
};
async function default_1(configOptions) {
    const config = await ObjectConfigLoader_1.ObjectConfigLoader.load(Object.assign({}, DEFAULTS, configOptions));
    const confluenceAPI = new ConfluenceAPI_1.ConfluenceAPI(config.baseUrl, config.authorizationToken, configOptions.insecure);
    for (const pageData of config.pages) {
        await (0, UpdatePage_1.updatePage)(confluenceAPI, pageData, config, configOptions.force);
    }
}
...
function setup(env) {
	...
	createDebug.humanize = __webpack_require__(6647);
...
if (typeof process === 'undefined' || process.type === 'renderer' || process.browser === true || process.__nwjs) {
	module.exports = __webpack_require__(7451);
} else {
	module.exports = __webpack_require__(6423);
}
...
try {
	...
	const supportsColor = __webpack_require__(9823);
...
var gOPD = __webpack_require__(2190);
...
var util = __webpack_require__(9023);
var isArrayish = __webpack_require__(9336);
...
var GetIntrinsic = __webpack_require__(4529);

var $defineProperty = GetIntrinsic('%Object.defineProperty%', true);

var hasToStringTag = __webpack_require__(944)();
var hasOwn = __webpack_require__(4403);
var $TypeError = __webpack_require__(7901);
...
const locatePath = __webpack_require__(2917);
...
module.exports = function () {
  if (!debug) {
    try {
      ...
      debug = __webpack_require__(8263)("follow-redirects");
    }
    catch (error) { /* */ }
...
var debug = __webpack_require__(8155);
...
var CombinedStream = __webpack_require__(162);
var util = __webpack_require__(9023);
...
var mime = __webpack_require__(2922);
var asynckit = __webpack_require__(9591);
var setToStringTag = __webpack_require__(8310);
var hasOwn = __webpack_require__(4403);
var populate = __webpack_require__(2172);
...
var implementation = __webpack_require__(8295);
...
var $Object = __webpack_require__(3272);

var $Error = __webpack_require__(1929);
var $EvalError = __webpack_require__(8259);
var $RangeError = __webpack_require__(6436);
var $ReferenceError = __webpack_require__(4192);
var $SyntaxError = __webpack_require__(2646);
var $TypeError = __webpack_require__(7901);
var $URIError = __webpack_require__(7043);

var abs = __webpack_require__(7449);
var floor = __webpack_require__(5611);
var max = __webpack_require__(2331);
var min = __webpack_require__(3673);
var pow = __webpack_require__(2131);
var round = __webpack_require__(9341);
var sign = __webpack_require__(7068);
...
var $gOPD = __webpack_require__(2190);
var $defineProperty = __webpack_require__(9283);
...
var hasSymbols = __webpack_require__(583)();

var getProto = __webpack_require__(2977);
var $ObjectGPO = __webpack_require__(8673);
var $ReflectGPO = __webpack_require__(6643);
...
var reflectGetProto = __webpack_require__(6643);
var originalGetProto = __webpack_require__(8673);

var getDunderProto = __webpack_require__(1535);
...
/** @type {import('.')} */
var $gOPD = __webpack_require__(6762);
...
var fs = __webpack_require__(9896)
var polyfills = __webpack_require__(7805)
var legacy = __webpack_require__(366)
var clone = __webpack_require__(7340)
...
var hasSymbolSham = __webpack_require__(53);
...
const path = __webpack_require__(6928);
const fs = __webpack_require__(6768);
const stripBom = __webpack_require__(3414);
const parseJson = __webpack_require__(9396);
const pify = __webpack_require__(1914);
...
const path = __webpack_require__(6928);
const pathExists = __webpack_require__(7512);
const pLocate = __webpack_require__(5157);
...
var $isNaN = __webpack_require__(3940);
...
var db = __webpack_require__(6191)
...
const pTry = __webpack_require__(4310);
...
const pLimit = __webpack_require__(6561);
...
const errorEx = __webpack_require__(1625);
const fallback = __webpack_require__(1254);
...
const path = __webpack_require__(6928);
const findUp = __webpack_require__(1810);
const loadJsonFile = __webpack_require__(3343);
...
const Signale = __webpack_require__(5763);
...
const chalk = __webpack_require__(7665);
const figures = __webpack_require__(7215);
const pkgConf = __webpack_require__(8238);
const pkg = __webpack_require__(4101);
const defaultTypes = __webpack_require__(8197);
...
const hasFlag = __webpack_require__(5614);
